...
async def post(
    ...
    try:
        ...
        await db_session.rollback()  # Rollback em caso de erro de integridade
        if 'cpf' in str(e.orig):  # Verifica se o erro é relacionado ao CPF
            raise HTTPException(
                status_code=303, 
        ...
        await db_session.rollback()  # Rollback em caso de outros erros
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail='Ocorreu um erro ao inserir os dados no banco'
        )
